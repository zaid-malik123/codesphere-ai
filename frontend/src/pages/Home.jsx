...
const Home = () => {
  ...
  const fetchProjects = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/project/all`,
        {
          withCredentials: true,
        }
      );
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BASE_URL}/api/users/all`, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  // Submit selected collaborators
  const handleAddCollaborators = async () => {
     await axios.put(
      `${import.meta.env.VITE_BASE_URL}/api/project/add-user`,
      { projectId: selectedProjectId, users: selectedUsers },
      { withCredentials: true }
    );
    await fetchProjects();
  ...
  return (
    <div className="bg-black min-h-screen w-full">
      ...
      {showAddUserModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 w-full max-w-lg rounded-lg p-6 shadow-lg">
            <h2 className="text-2xl font-bold text-white mb-4">
              Add Collaborators
            </h2>
            ...
            <div className="max-h-80 overflow-y-auto space-y-3">
              {allUsers && allUsers.length > 0 ? (
                allUsers.map((user) => (
                  <div
                    key={user._id}
                    className="flex items-center gap-3 bg-gray-700 p-3 rounded-lg border border-gray-600 hover:border-blue-500 transition"
                  >
                    {/* Avatar */}
                    ...
                    {/* Email */}
                    <div className="flex-1 text-gray-200">{user.email}</div>
